...
class ProfilePageState extends State<ProfilePage> {
  ...
  // ── Kullanıcı seviyesi hesapla (Aura bazlı — 12 Ünvan) ──
  // Her ünvan için özel ikon üretilecek (şimdilik Material Icon placeholder).
  // Başlarda kolay, sonlara doğru katlanarak zorlaşan eşikler.
  ({IconData icon, Color color, String title}) _getUserLevel(String lang) {
    final int aura = _bonusAura;
    
    if (lang == 'tr') {
      if (aura < 50)     return (icon: Icons.eco_rounded,                    color: const Color(0xFF4ADE80), title: 'Tohum');
      if (aura < 150)    return (icon: Icons.nightlight_round,               color: const Color(0xFFFBBF24), title: 'Kıvılcım');
      if (aura < 400)    return (icon: Icons.cloud_rounded,                  color: const Color(0xFF94A3B8), title: 'Sezgi');
      if (aura < 1000)   return (icon: Icons.auto_awesome_rounded,           color: const Color(0xFFA78BFA), title: 'Alev');
      if (aura < 2500)   return (icon: Icons.star_rounded,                   color: const Color(0xFF38BDF8), title: 'Yıldız');
      if (aura < 5000)   return (icon: Icons.bedtime_rounded,                color: const Color(0xFFC084FC), title: 'Kahin');
      if (aura < 10000)  return (icon: Icons.visibility_rounded,             color: const Color(0xFFF97316), title: 'Büyücü');
      if (aura < 18000)  return (icon: Icons.diamond_rounded,                color: const Color(0xFFE879F9), title: 'Bilge');
      if (aura < 30000)  return (icon: Icons.remove_red_eye_rounded,         color: const Color(0xFF6366F1), title: 'Gölge');
      if (aura < 50000)  return (icon: Icons.bolt_rounded,                   color: const Color(0xFFEC4899), title: 'Üstat');
      if (aura < 100000) return (icon: Icons.whatshot_rounded,               color: const Color(0xFFF43F5E), title: 'Efsane');
      return             (icon: Icons.workspace_premium_rounded,             color: const Color(0xFFFFD700), title: 'Kozmik');
    } else {
      if (aura < 50)     return (icon: Icons.eco_rounded,                    color: const Color(0xFF4ADE80), title: 'Seed');
      if (aura < 150)    return (icon: Icons.nightlight_round,               color: const Color(0xFFFBBF24), title: 'Spark');
      if (aura < 400)    return (icon: Icons.cloud_rounded,                  color: const Color(0xFF94A3B8), title: 'Sense');
      if (aura < 1000)   return (icon: Icons.auto_awesome_rounded,           color: const Color(0xFFA78BFA), title: 'Flame');
      if (aura < 2500)   return (icon: Icons.star_rounded,                   color: const Color(0xFF38BDF8), title: 'Star');
      if (aura < 5000)   return (icon: Icons.bedtime_rounded,                color: const Color(0xFFC084FC), title: 'Seer');
      if (aura < 10000)  return (icon: Icons.visibility_rounded,             color: const Color(0xFFF97316), title: 'Mage');
      if (aura < 18000)  return (icon: Icons.diamond_rounded,                color: const Color(0xFFE879F9), title: 'Sage');
      if (aura < 30000)  return (icon: Icons.remove_red_eye_rounded,         color: const Color(0xFF6366F1), title: 'Shadow');
      if (aura < 50000)  return (icon: Icons.bolt_rounded,                   color: const Color(0xFFEC4899), title: 'Master');
      if (aura < 100000) return (icon: Icons.whatshot_rounded,               color: const Color(0xFFF43F5E), title: 'Legend');
      return             (icon: Icons.workspace_premium_rounded,             color: const Color(0xFFFFD700), title: 'Cosmic');
    }
  }
